...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="car-card group">
      ...
      <p className="flex mt-6 text-[32px] leading-[38px] font-extrabold justify-between w-full">
...
2
        ...
        {product.price}
        <span className="self-end text-[14px] leading-[17px] font-medium mb-1">
     {product && isArticleInContext(product._id) ? <button
     onClick={()=>removeEntireItem(product._id)}
                  class="p-1 transition text-sm ml-8 ease-in duration-200 uppercase rounded-full hover:bg-gray-800 hover:text-white border-2 border-gray-900 focus:outline-none">Remove</button> : <button
                  
                  onClick={()=>addToReadLater(product._id, product.publisher, product.title, product.image)}
                  class="p-1 mt-[-2] transition text-sm ml-8 ease-in duration-200 uppercase rounded-full hover:bg-gray-800 hover:text-white border-2 border-gray-900 focus:outline-none">Read Later</button>}
            </span>
      </p>
