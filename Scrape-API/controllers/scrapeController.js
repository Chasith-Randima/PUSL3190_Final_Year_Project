...
// controller function for scraping articles from news first publisher
exports.scrapeNewsFirst = catchAsync(async (req, res, next) => {
    try {
        ...
        const page = await browser.newPage();
        // setting tab size and other features we want in the tab as we needed
        await page.setViewport({ width: 1600, height: 1000, isMobile: false, isLandscape: true, hasTouch: false, deviceScaleFactor: 1 });

        // going to the main website
        await page.goto("https://english.newsfirst.lk/latest");

        // Wait for the selector to be available before trying to access data
        await page.waitForSelector(".local_news_main div .ng-star-inserted");
                    ...
                    try {
                        ...
                        if (pageTitleElement) {
                            ...
                            if (authorElement) {
                                ...
                                extractedData.push({ title: pageTitle, author: authorName, date, time, body ,summarized:"false",url:url });
                            }
                        }
            ...
            try {
                ...
                if (!tempTitle) {
                    fetch("http://127.0.0.1:3005/auto", {
                        ...
                        body:JSON.stringify({content: formatBodyText(item.body),title:item.title,author:item.author,publisher:"newsFirst",customFetch:"false",url:item.url})
                      })
    ...
    try {
        ...
        if (!tempTitle) {
            fetch("http://127.0.0.1:3005/auto", {
                ...
                // body: {content:item.body},
                body:JSON.stringify({content: formatBodyText(content.body),title:content.title,author:"item.author",publisher:"adaderana",customFetch:"true",url:link})
              }).then((res)=>{
                ...
                if (!tempTitle) {
                    fetch("http://127.0.0.1:3005/auto", {
                        ...
                        body:JSON.stringify({content: formatBodyText(item.body),title:item.title,
                            ...
                            time:item.time,
                            // image:item.image,
                            publisher: "adaderana",customFetch:"false", url:item.url }),
    ...
    try {
        ...
        for (const link of links) {
            try {
                ...
                scrapedData.push({...content,url:link});
            ...
            try {
                ...
                if (!tempTitle) {
                    fetch("http://127.0.0.1:3005/auto", {
                        ...
                        // body: {content:item.body},
                        body:JSON.stringify({content: formatBodyText(item.body),title:item.title,author:item.author,date:item.date,time:item.time,image:item.image,publisher: "bbc",customFetch:"false",url:item.url})
                      })
...
exports.scrapeTheSun = catchAsync(async (req, res, next) => {
    try {
       ...
       // Wait for the selector to be available before trying to access data
        ...
        // Extracting links and storing them in links array
        const links = await page.evaluate(() => {
        ...
        // initialing array to store scraped data object
        const scrapedData = [];
        ...
        for (const link of links) {
            try {
                // opening a new tab
                const newPage = await browser.newPage();
                // Retry navigation on timeout and promisifying page load
                await Promise.race([
                    // going to given link in new tab
                    newPage.goto(link, { waitUntil: 'domcontentloaded' }),
                    // increasing time out function
                    new Promise((resolve, reject) => setTimeout(reject, 120000)) 
                ]);
                ...
                const content = await newPage.evaluate(() => {
                    ...
                    // getting all the paragraphs wich has news content
                    const paragraphElements = document.querySelectorAll('#main-content > section > div > main > article > div.article__content p');
                    // initializing variable for storing concatenated paragraphs
                    let concatenated = '';
                    // iterating through all pragraphs and creating one single paragraph for ease of summarizing
                    paragraphElements.forEach((element, index) => {
                        concatenated += element.textContent.trim();
                        if (index < paragraphElements.length - 1) {
                            concatenated += '\n';
                    ...
                    // preprocessing scraped data before saving
                    const title = titleElement ? titleElement.textContent.trim() : "";
                    const author = authorElement ? authorElement.textContent.trim() : "";
                    const body = concatenated;
                    const date = dateElement ? dateElement.getAttribute('datetime').split("T")[0] : "";
                    ...
                    // returning object including all the scraped data
                    return { title, author, body, date, time, image };
                });

                // pushing the scraped data to temporary array that holds scraped content along with url
                scrapedData.push({...content,url:link});
                // closing the opened tab for individual article
                await newPage.close();
                // catching aerrors and console loging them
            } catch (error) {
                console.error("Error navigating to:", link, error);
                // Continue to the next link even if the scraping from current link failed
                continue;
            }
        }
        // After iterating through all the links and scraping data closing the entire browser
        await browser.close();

        // iterating trough all the scraped data , saving them and summarizing them
        for (const item of scrapedData) {
            try {
                // getting if there is a article saved with same title
                let tempTitle = await ScrapedArticle.findOne({ articleTitle: item.title });

                // if there are no article saved with same title
                if (!tempTitle) {
                    // sending scraped data to summarization api to be summarized
                    fetch("http://127.0.0.1:3005/auto", {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json"
                        },
                        body:JSON.stringify({content: formatBodyText(item.body),title:item.title,author:item.author,date:item.date,time:item.time,image:item.image, publisher: "thesun",customFetch:"false",url:item.url })
                      })
                    //   saving raw data to database for future use cases
                ...
                // If there is a article with same title console loging it already exisits in the database
                } else {
                    console.log("article already in the database...", tempTitle.title)
                }
            // if there is a error in overall process console loging them
            } catch (error) {
                console.error("Error saving scraped data to database:", error);
            }
        }

        // checking whether this request is to fetch articles from this article only or to scrape from all other publishers as well
        if (req.query?.all == "all") {
            // if so going to next publisher to scrape articles
            console.log("next is about to be called...")
            next()
        } else {
            // else returning success status
            res.status(200).json({
                message: "success"
            })
        }
    // Catching if there is a error in overall process and console loging them 
    } catch (error) {
        console.error("Error:", error);
    }
});
            ...
            try {
                ...
                if (!tempTitle) {
                    fetch("http://127.0.0.1:3005/auto", {
                        ...
                        // body: {content:item.body},
                        body:JSON.stringify({content: formatBodyText(item.body),title:item.title,author:item.author,date:item.date,time:item.time,image:item.image,publisher: "hindustantimes",customFetch:"false",url:item.url })
                      })
            ...
            try {
                ...
                if (!tempTitle) {
                    fetch("http://127.0.0.1:3005/auto", {
                        ...
                        // body: {content:item.body},
                        body:JSON.stringify({content: formatBodyText(item.body),title:item.title,author:item.author,date:item.date,time:item.time,image:item.image,publisher: "nikkei",customFetch:"false" ,url:item.url})
                      })
                    await ScrapedArticle.create({
                        articleTitle: item.title,
                        content: item.body,
...
exports.scrapeOneNewsFirst = catchAsync(async (req, res, next) => {
    try {
       // Launching puppeteer browser
        ...
        // Setting tab size and other features we want in the tab as we needed
        await page.setViewport({ width: 1600, height: 1000, isMobile: false, isLandscape: true, hasTouch: false, deviceScaleFactor: 1 });
        // Taking the custom url from the request body
        ...
        // closing the browser after scraping data
        await browser.close();

        try {
            // Check whether article with same title exists on the data base
            const existingArticle = await ScrapedArticle.findOne({ articleTitle: data.title });
            // If there is no article with the same title
            if (!existingArticle) {
                //Sending scraped article data to summarization API
            ...
            // If article already exists in the database
            } else {
                // returning response with the message
            ...
            // catching any erros with saving scraped data or sending data to summarization API and console login them
        } catch (error) {
            console.error("Error saving scraped data to database:", error);
        }
        // Catching if there are any errors in the entire code and logging them
    } catch (error) {
        console.error("Error:", error);
    }
});
