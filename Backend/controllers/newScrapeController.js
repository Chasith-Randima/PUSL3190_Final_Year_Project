...
exports.scrapeOneNewsFirst = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        const fetchResponse = await fetch("http://127.0.0.1:9000/bart", {
            method: "POST",
            headers: {
...
//     fetch("http://127.0.0.1:9000/bart", {
//         method: "POST",
//         headers: {
//             "Content-Type": "application/json"
...
exports.scrapeNewsFirst = catchAsync(async (req, res, next) => {
    console.log("route is called new  news first....", req.query);
    try {
        const browser = await puppeteer.launch();
                    ...
                    try {
                        ...
                        if (pageTitleElement) {
                            ...
                            if (authorElement) {
                                ...
                                console.log("working...news firtst")
                                ...
                                //         method: "POST",
                                //         body: JSON.stringify({ inputs: body }) // Send the title for summarization
                                //     }
                                // );
                                ...
                                //       return err;
                                //     });
                                // extractedData.push({ title: pageTitle, author: authorName, date, time, body ,summarized:"false",summary:response.json()[0].summary_text });
                                extractedData.push({ title: pageTitle, author: authorName, date, time, body ,summarized:"false" });

                                // let createdArticle = await Article.create({
                                //     original_text: body,
                                ...
                                console.log("This is the created articles----------",createdArticle)
                            }
        ...
        //     let createdArticle = await ScrapedArticle.create(item)
        //     console.log(createdArticle)

        // }
        for (const item of data) {
            console.log("inside for loop working..")
            try {
                let tempTitle = await ScrapedArticle.findOne({ articleTitle: item.title });

                 // fetch(`${API}/bart`, {
                    // fetch("http://127.0.0.1:9000/bart", {
                    // fetch("http://127.0.0.1:9000/auto", {
                    //     method: "POST",
                    //     headers: {
                    //         "Content-Type": "application/json"
                    //         // Accept: "application/json",
                    ...
                    //     },
                    //     // body: {content:item.body},
                    //     body:JSON.stringify({content: formatBodyText(item.body)})
                    //   })
                        // .then((response) => {
                        //   return response.json();
                        // })
...
exports.scrapeOneAdaDerana = catchAsync(async (req, res, next) => {
    ...
    try {
        ...
        // Call the fetch function with the scraped content
        const response = await fetch("http://127.0.0.1:9000/bart", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({ content: content.body })
        });
